...
def enable_github_pages(repo_full_name, branch_name):
    ...
    if resp.status_code in [201, 204]:
        return f"https://{repo_full_name.split('/')[0]}.github.io/{repo_full_name.split('/')[1]}/"
    else:
        print(f"⚠️ Failed to enable GitHub Pages: {resp.status_code} {resp.text}")
        return None


def push_files_to_github(files: dict, task_name: str, evaluation_url: str, commit_message="Generated by AI"):
    def notify_evaluation_url(evaluation_url, task_name, branch_name, repo_url, status="completed"):
        try:
            payload = {
                "task": task_name,
                "branch": branch_name,
            ...
            print(f"✅ Notification sent to {evaluation_url}")
        except Exception as e:
            print(f"⚠️ Failed to notify evaluation_url: {e}")
    ...
    try:
        git_page_url = enable_github_pages(repo.full_name, branch_name)
    except Exception as e:
...
async def receive_task_endpoint(request: Request):
    ...
    try:
        ...
        repo_url = push_files_to_github(files,task ,urleval, commit_message=f"Task: {task_description[:30]}")
